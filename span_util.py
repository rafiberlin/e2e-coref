...
def get_parent_child_emb(clusters, span_emb, span_starts, span_ends, label_type):
    ...
    for coref_relation, dist in clusters:
        assert len(coref_relation) == 2, 'Member of mentions are not equal to 2'  # np.where gives the indices back where it maches
        parent_idx = np.intersect1d(np.where(span_starts == coref_relation[0][0]), # I list everything where the beginning of the first mention start.
                                    np.where(span_ends == coref_relation[0][1]))# I list everything where the first mention ends. # YOur error: each entry in
